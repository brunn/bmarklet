...
if ($loggedon && isset($_POST['submitted'])) {
    if (!$_POST['title'] || !$_POST['address']) {
        $tplVars['error'] = T_('Järjehoidjal peab olema tiitel ja aadress');
        ...
        // If it's new, save it
            ...
            if ($bookmarkservice->addBookmark($address, $title, $description, $status, $categories)) {
                if (isset($_POST['popup'])) {
                    ...
                    $tplVars['msg'] = T_('Järjehoidja salvestatud');
                }
            } else {
...
if ($templatename == 'editbookmark.tpl') {
    if ($loggedon) {
       ...
       # $title = T_('Lisa järjehoidja');
        ...
        $tplVars['error'] = T_('Lingi lisamiseks logi sisse');
    }
    ...
    if ($user == $currentUsername) {
       # $title = T_('Minu lingid') . filter($catTitle);
    } else {
